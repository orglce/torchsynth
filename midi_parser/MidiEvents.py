from midi_parser.Util import Util
import math
...
class NoteOffEvent(ChannelEvent):
    ...
    def __str__(self):
        event_type = "Note Off"
        return f"{event_type:15s} {self.delta_time:-5d} Channel: {self.channel} Note Number: {self.note_number}"
...
class NoteOnEvent(ChannelEvent):
    ...
    def __str__(self):
        event_type = ("Note Off (On)" if self.is_note_off()
                      else "Note On")
        # return (event_type + ", Channel: " + str(self.channel) + " Note Number: " + str(self.note_number) + ", Velocity: " + str(self.velocity))
        return f"{event_type:15s} {self.delta_time:-5d} Channel: {self.channel} Note Number: {self.note_number}, Velocity: {self.velocity}"
